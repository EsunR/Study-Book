...
import { hexToRgba, shiftColor } from "./ThemeProvider";

/** 解析页面级样式上下文（兼容旧接口，从 ThemeColors 生成）*/
export function usePageContext(theme?: ThemeColors) {
  ...
  const subTextColor =
    theme?.textSecondary || (isDark ? "rgba(255,255,255,0.6)" : "rgba(0,0,0,0.6)");
  const cardBg = theme?.surface || (isDark ? "#252540" : "var(--card-bg)");
