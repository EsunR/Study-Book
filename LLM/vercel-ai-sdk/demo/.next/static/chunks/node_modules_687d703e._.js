...
function createToolNameMapping(param) {
    let { tools = [], providerToolNames, resolveProviderToolName } = param;
    var _a2;
...
function convertImageModelFileToDataUri(file) {
    ...
    return "data:".concat(file.mediaType, ";base64,").concat(typeof file.data === "string" ? file.data : convertUint8ArrayToBase64(file.data));
}
// src/convert-to-form-data.ts
function convertToFormData(input) {
    ...
    for (const [key, value] of Object.entries(input)){
        ...
        if (Array.isArray(value)) {
            ...
            const arrayKey = useArrayBrackets ? "".concat(key, "[]") : key;
...
async function readResponseWithSizeLimit(param) {
    let { response, url, maxBytes = DEFAULT_MAX_DOWNLOAD_SIZE } = param;
    ...
    if (contentLength != null) {
        ...
        if (!isNaN(length) && length > maxBytes) {
            throw new DownloadError({
                url,
                message: "Download of ".concat(url, " exceeded maximum size of ").concat(maxBytes, " bytes (Content-Length: ").concat(length, ").")
            });
        }
    }
    ...
    try {
        while(true){
            ...
            if (totalBytes > maxBytes) {
                throw new DownloadError({
                    url,
                    message: "Download of ".concat(url, " exceeded maximum size of ").concat(maxBytes, " bytes.")
                });
            }
            chunks.push(value);
        }
...
function validateDownloadUrl(url) {
    ...
    try {
        ...
        throw new DownloadError({
            url,
            message: "Invalid URL: ".concat(url)
        });
    }
    ...
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
        throw new DownloadError({
            url,
            message: "URL scheme must be http, https, or data, got ".concat(parsed.protocol)
        });
    }
    ...
    if (hostname === "localhost" || hostname.endsWith(".local") || hostname.endsWith(".localhost")) {
        throw new DownloadError({
            url,
            message: "URL with hostname ".concat(hostname, " is not allowed")
        });
    }
    if (hostname.startsWith("[") && hostname.endsWith("]")) {
        ...
        if (isPrivateIPv6(ipv6)) {
            throw new DownloadError({
                url,
                message: "URL with IPv6 address ".concat(hostname, " is not allowed")
            });
        }
        return;
    }
    if (isIPv4(hostname)) {
        if (isPrivateIPv4(hostname)) {
            throw new DownloadError({
                url,
                message: "URL with IP address ".concat(hostname, " is not allowed")
            });
        }
        return;
    }
    ...
    if (normalized.startsWith("::ffff:")) {
        ...
        if (hexParts.length === 2) {
            ...
            if (!isNaN(high) && !isNaN(low)) {
                ...
                const c = low >> 8 & 255;
                const d = low & 255;
                return isPrivateIPv4("".concat(a, ".").concat(b, ".").concat(c, ".").concat(d));
            }
        }
    }
...
function handleFetchError(param) {
    ...
    if (isBunNetworkError(error)) {
        return new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$ai$2d$sdk$2f$provider$2f$dist$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["APICallError"]({
            message: "Cannot connect to API: ".concat(error.message),
            cause: error,
            url,
            requestBodyValues,
            isRetryable: true
...
// src/version.ts
var VERSION = ("TURBOPACK compile-time truthy", 1) ? "4.0.22" : "TURBOPACK unreachable";
// src/get-from-api.ts
var getOriginalFetch = ()=>globalThis.fetch;
var getFromApi = async (param)=>{
    let { url, headers = {}, successfulResponseHandler, failedResponseHandler, abortSignal, fetch: fetch2 = getOriginalFetch() } = param;
    try {
...
async function validateTypes(param) {
    let { value, schema, context } = param;
...
var postJsonToApi = async (param)=>{
    let { url, headers, body, failedResponseHandler, successfulResponseHandler, abortSignal, fetch: fetch2 } = param;
    return postToApi({
        url,
        headers: {
...
var postFormDataToApi = async (param)=>{
    let { url, headers, formData, failedResponseHandler, successfulResponseHandler, abortSignal, fetch: fetch2 } = param;
    return postToApi({
        url,
        headers,
        body: {
...
var postToApi = async (param)=>{
    let { url, headers = {}, body, successfulResponseHandler, failedResponseHandler, abortSignal, fetch: fetch2 = getOriginalFetch2() } = param;
    try {
        const response = await fetch2(url, {
            method: "POST",
            headers: withUserAgentSuffix(headers, "ai-sdk/provider-utils/".concat(VERSION), getRuntimeEnvironmentUserAgent()),
...
function createProviderToolFactory(param) {
    let { id, inputSchema } = param;
    return (param)=>{
        let { execute, outputSchema, needsApproval, toModelOutput, onInputStart, onInputDelta, onInputAvailable, ...args } = param;
        return tool({
            type: "provider",
            id,
            args,
...
function createProviderToolFactoryWithOutputSchema(param) {
    let { id, inputSchema, outputSchema, supportsDeferredResults } = param;
    return (param)=>{
        let { execute, needsApproval, toModelOutput, onInputStart, onInputDelta, onInputAvailable, ...args } = param;
        return tool({
            type: "provider",
            id,
            args,
